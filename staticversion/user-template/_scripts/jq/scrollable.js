...
(function($) {
	...
	function Scrollable(el, config) {   
		...
		// RG - modified this section so width works with variable widths
		if (opts.horizontal) {
			itemRoot.width(this.root.width() - $(opts.prev).width() - $(opts.next).width() - 20);
		} else {
	...
	$.extend(Scrollable.prototype, {
		...
		seekTo: function(index, time) {
			...
			var max = this.items.length - this.opts.size;
			if (index > max) {
				index = max;
			}
	...
	// RG - Used for tab handler
	function hasParentClass(obj, className) {
		if(obj.className == className) {
			return true;
		}
		else if(obj.parentNode) {
			return hasParentClass(obj.parentNode, className)
		}
		return false;
	}
	...
	$(window).bind("keypress.scrollable", function(evt) {
		...
		if (el.opts.horizontal && evt.ctrlKey && (evt.keyCode == 37 || evt.keyCode == 39)) {
			el.move(evt.keyCode == 37 ? -1 : 1);
			return true;
		}

		if (!el.opts.horizontal && evt.ctrlKey && (evt.keyCode == 38 || evt.keyCode == 40)) {
			el.move(evt.keyCode == 38 ? -1 : 1);
			return true;
		}

		// RG - Added tab key code handler
		if(evt.keyCode == 9 && hasParentClass(evt.originalTarget, '__scrollable')) {
			el.move(evt.shiftKey ? -1 : 1);
			return true;
		}
		return true;
