...
function onEnterEditTab(Tab $_tab, $_page, $sortMethod)
{
	...
	$editMenu = new Menu( array(
								Link::CreateIconLink('Finish', $page->PathWithQueryString(array('mode'=>Page::MODE_SHOW)), $studentTheme->Icon('stop-abort'), array('title'=>"Finish editing {$tab->getName()} tab", 'alt' => 'Finish editing')),
								Link::CreateIconLink('Add', "page-0?tab={$tab->getId()}", $studentTheme->Icon('add-page'), array('title'=>'Add new page', 'alt' => 'Add new page'))
								),
							'menu_tabs');
...
function onEnterShowTab(Tab $_tab, SimplePage $_page, $sortMethod)
{
	...
	if($tab->getId() != null) {
		...
		if($tab->getTemplate() == null) {
			$editMenu->addLink( Link::CreateIconLink('Edit', $page->PathWithQueryString(array('mode'=>Page::MODE_EDIT)), $studentTheme->Icon('edit-page'), array('title'=>"Edit {$tab->getName()} tab", 'alt' => 'Edit')) );
		}
	}
}
...
// Set up events
$eventD = new TabEventDispatcher($_GET, $_POST);
